...
**Hard limit / do not chase perfection:** because the card self-centers and resizes, no static crop is pixel-perfect at every width — odd in-between window sizes can still show a thin slate sliver or faint corner. The embed's navy header has rounded TOP corners whose radius overlaps the title's vertical position, so cropping deep enough to hide the corner gaps clips the title letters; cropping only the slate band leaves corner gaps. The fallback resolves this by cropping just the slate band and masking the two top corners with `bg-navy` overlay squares (`rounded-tl/tr-xl`).

**True fix — auto-height contract (chosen by user):** the parent (`landing.tsx`) listens for `window.postMessage({ type: "hfl-embed-height", height: <px> }, ...)` from the CRM origin and sets the iframe to that exact height (state `formHeight`), dropping the crop + corner masks (renders flush). This requires the CRM `/embed/form` page to (a) render the card flush/square with no slate body padding or min-h-screen centering, and (b) post its `document.documentElement.scrollHeight` on load and via a `ResizeObserver` (catches step1->step2 height change). Until a message arrives, the cropped fallback runs. The CRM is a SEPARATE app not in this workspace, so this half must be applied there.

**Why:** auto-height is the only way to get both no-internal-scroll AND no-empty-space across the short step 1 and taller step 2; a single static height cannot satisfy both.
